...
export default function PortfolioGridItem({ project }: Props) {
  return (
    <Link
      href={`/portfolio/${project.code}`}
      className='group relative w-full h-60 mb-14 sm:mb-0 cursor-pointer'
    >
      <Image
        ...
        priority={project.title === 'proalco'}
      />
      ...
      <h6 className='absolute w-full sm:hidden text-center uppercase text-white text-lg font-semibold my-4'>
        <span className='pb-1 border-b border-customDarkGray-400'>{project.title}</span>
      </h6>
    </Link>
  );
}
